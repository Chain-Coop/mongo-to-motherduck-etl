...
from bson import ObjectId  # ✅ Needed to convert string back to ObjectId

def sync_manualsavings():
    load_dotenv()
    ...
    try:
        ...
        collection = db["manualsavings"]
        print("✅ Successfully connected to MongoDB!")
    except Exception as e:
        print("❌ MongoDB Error:", e)
        return

    # Optional debug: Count documents in MongoDB
    ...
    # Step 4: Check if 'manualsavings' table exists
    table_exists = md_conn.execute("""
        SELECT COUNT(*) 
        FROM duckdb_tables() 
        WHERE table_name = 'manualsavings'
    """).fetchone()[0] > 0
    ...
    if table_exists:
        try:
            ...
            last_synced_contributions = md_conn.execute(
                "SELECT mongo_id FROM manualsavings ORDER BY mongo_id DESC LIMIT 1"
            ).fetchone()[0]

            if last_synced_contributions and last_synced_contributions:
                last_id = ObjectId(last_synced_contributions)  # Convert string to ObjectId
    ...
    if df.empty:
        print("ℹ️ No manualsavings found in MongoDB.")
        return

    print(f"📥 Pulled documents from MongoDB: {df.shape[0]}")
    ...
    existing_ids = []
    try:
        existing_ids = md_conn.execute("SELECT mongo_id FROM manualsavings").fetchdf()["mongo_id"].tolist()
    except duckdb.CatalogException:
        print("ℹ️ No existing manualsavings table found. All records will be synced.")

    df = df[~df["mongo_id"].isin(existing_ids)]

    # ✅ Exit if no new records after deduplication
    if df.empty:
        print("ℹ️ No new manualsavings to sync.")
        return
    ...
    md_conn.execute("CREATE TABLE IF NOT EXISTS manualsavings AS SELECT * FROM df LIMIT 0")
    md_conn.execute("""
        INSERT INTO manualsavings
        SELECT * FROM df
        WHERE mongo_id NOT IN (SELECT mongo_id FROM manualsavings)
    """)
