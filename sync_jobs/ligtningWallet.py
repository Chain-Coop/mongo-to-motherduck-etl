...
def sync_lndwallets():
    ...
    # Step 4: Check if 'lndwallets' table exists in DuckDB
    table_exists = md_conn.execute("""
        SELECT COUNT(*) 
        FROM duckdb_tables() 
        WHERE table_name = 'lndwallets'
    """).fetchone()[0] > 0
    ...
    if table_exists:
        try:
            ...
            last_synced_contributions = md_conn.execute(
                "SELECT mongo_id FROM lndwallets ORDER BY mongo_id DESC LIMIT 1"
            ).fetchone()[0]
    ...
    if df.empty:
        print("ℹ️ No lndwallets found in MongoDB.")
        return
    ...
    existing_ids = []
    try:
        existing_ids = md_conn.execute("SELECT mongo_id FROM lndwallets").fetchdf()["mongo_id"].tolist()
    except duckdb.CatalogException:
        print("ℹ️ No existing lndwallets table found. All records will be synced.")

    df = df[~df["mongo_id"].isin(existing_ids)]

    # ✅ Exit if no new records after deduplication
    if df.empty:
        print("ℹ️ No new lndwallets to sync.")
        return
    ...
    md_conn.execute("CREATE TABLE IF NOT EXISTS lndwallets AS SELECT * FROM df LIMIT 0")
    md_conn.execute("""
        INSERT INTO lndwallets
        SELECT * FROM df
        WHERE mongo_id NOT IN (SELECT mongo_id FROM lndwallets)
    """)

    print("✅ LND Wallets data successfully pushed to MotherDuck!")
