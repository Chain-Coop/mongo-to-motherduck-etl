...
from bson import ObjectId  # Required to handle ObjectId for MongoDB
...
def sync_wallets():
    ...
    # Step 4: Check if 'wallets' table exists in DuckDB
    ...
    if table_exists:
        try:
            ...
            last_synced_contributions = md_conn.execute(
                "SELECT mongo_id FROM wallets ORDER BY mongo_id DESC LIMIT 1"
            ).fetchone()[0]
    ...
    if df.empty:
        print("ℹ️ No wallets found in MongoDB.")
        return
    ...
    existing_ids = []
    try:
        existing_ids = md_conn.execute("SELECT mongo_id FROM wallets").fetchdf()["mongo_id"].tolist()
    except duckdb.CatalogException:
        print("ℹ️ No existing wallets table found. All records will be synced.")

    df = df[~df["mongo_id"].isin(existing_ids)]

    # ✅ Exit if no new records after deduplication
    if df.empty:
        print("ℹ️ No new wallets to sync.")
        return
    ...
    md_conn.execute("CREATE TABLE IF NOT EXISTS wallets AS SELECT * FROM df LIMIT 0")
    md_conn.execute("""
        INSERT INTO wallets
        SELECT * FROM df
        WHERE mongo_id NOT IN (SELECT mongo_id FROM wallets)
    """)

    print("✅ Wallets data successfully pushed to MotherDuck!")
