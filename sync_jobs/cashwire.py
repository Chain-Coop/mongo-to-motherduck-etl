...
def sync_cashwyretransactions():
    ...
    try:
        ...
        collection = db["cashwyretransactions"]
    ...
    # Step 4: Check if 'cashwyretransactions' table exists in DuckDB
    table_exists = md_conn.execute("""
        SELECT COUNT(*) 
        FROM duckdb_tables() 
        WHERE table_name = 'cashwyretransactions'
    """).fetchone()[0] > 0
    ...
    if table_exists:
        try:
            ...
            last_synced_contributions = md_conn.execute(
                "SELECT mongo_id FROM cashwyretransactions ORDER BY mongo_id DESC LIMIT 1"
            ).fetchone()[0]
    ...
    if df.empty:
        print("ℹ️ No cashwyretransactions found in MongoDB.")
        return
    ...
    existing_ids = []
    try:
        existing_ids = md_conn.execute("SELECT mongo_id FROM cashwyretransactions").fetchdf()["mongo_id"].tolist()
    except duckdb.CatalogException:
        print("ℹ️ No existing cashwyretransactions table found. All records will be synced.")

    df = df[~df["mongo_id"].isin(existing_ids)]

    # ✅ Exit if no new records after deduplication
    if df.empty:
        print("ℹ️ No new cashwyretransactions to sync.")
        return
    ...
    md_conn.execute("CREATE TABLE IF NOT EXISTS cashwyretransactions AS SELECT * FROM df LIMIT 0")
    md_conn.execute("""
        INSERT INTO cashwyretransactions
        SELECT * FROM df
        WHERE mongo_id NOT IN (SELECT mongo_id FROM cashwyretransactions)
    """)

    print("✅ Cashwyre transactions data successfully pushed to MotherDuck!")
