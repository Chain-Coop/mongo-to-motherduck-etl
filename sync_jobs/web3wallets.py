...
def sync_web3wallets():
    ...
    # Step 4: Check if 'web3wallets' table exists in DuckDB
    table_exists = md_conn.execute("""
        SELECT COUNT(*) 
        FROM duckdb_tables() 
        WHERE table_name = 'web3wallets'
    """).fetchone()[0] > 0
    ...
    if table_exists:
        try:
            ...
            last_synced_contributions = md_conn.execute(
                "SELECT mongo_id FROM web3wallets ORDER BY mongo_id DESC LIMIT 1"
            ).fetchone()[0]
    ...
    if df.empty:
        print("ℹ️ No web3wallets found in MongoDB.")
        return
    ...
    existing_ids = []
    try:
        existing_ids = md_conn.execute("SELECT mongo_id FROM web3wallets").fetchdf()["mongo_id"].tolist()
    except duckdb.CatalogException:
        print("ℹ️ No existing web3wallets table found. All records will be synced.")

    df = df[~df["mongo_id"].isin(existing_ids)]

    # ✅ Exit if no new records after deduplication
    if df.empty:
        print("ℹ️ No new web3wallets to sync.")
        return
    ...
    md_conn.execute("CREATE TABLE IF NOT EXISTS web3wallets AS SELECT * FROM df LIMIT 0")
    md_conn.execute("""
        INSERT INTO web3wallets
        SELECT * FROM df
        WHERE mongo_id NOT IN (SELECT mongo_id FROM web3wallets)
    """)

    print("✅ Web3 wallets data successfully pushed to MotherDuck!")
